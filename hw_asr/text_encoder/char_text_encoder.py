...
class CharTextEncoder(BaseTextEncoder):
    ...
    def __getitem__(self, item: int):
        assert type(item) is int, f'CharTextEncoder has invalid key type: {type(item)}; must be int.'
    ...
    def decode(self, vector: Union[Tensor, np.ndarray, List[int]]):
        try:
            ...
            unknown_ind = set([idx for idx in vector if idx not in self.ind2char])
            raise Exception(
                f"Can't decode vector '{vector}'. Unknown indices: '{' '.join(unknown_ind)}'")
